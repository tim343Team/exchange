...
public class ChatAdapter extends BaseAdapter {
    ...
    void initRowItems() {
        //R代表收到的消息；T代表发出的消息
        chatRowHashMap.put(Integer.valueOf(1), new TextRxChatRow(1));//收到的文本消息
        chatRowHashMap.put(Integer.valueOf(2), new TextTxChatRow(2));//发出的文本消息
        chatRowHashMap.put(Integer.valueOf(3), new ImageRxChatRow(3));//图片消息
        chatRowHashMap.put(Integer.valueOf(4), new ImageTxChatRow(4));
        chatRowHashMap.put(Integer.valueOf(5), new VoiceRxChatRow(5));//语音消息
        chatRowHashMap.put(Integer.valueOf(6), new VoiceTxChatRow(6));
        chatRowHashMap.put(Integer.valueOf(7), new InvestigateChatRow(7));//评价类型消息
        chatRowHashMap.put(Integer.valueOf(8), new FileRxChatRow(8));//文件消息
        chatRowHashMap.put(Integer.valueOf(9), new FileTxChatRow(9));
        chatRowHashMap.put(Integer.valueOf(10), new IframeRxChatRow(10));//网页消息
        chatRowHashMap.put(Integer.valueOf(11), new BreakTipChatRow(11));//断开提示消息
        chatRowHashMap.put(Integer.valueOf(12), new TripRxChatRow(12));//度假消息
        chatRowHashMap.put(Integer.valueOf(13), new RichRxChatBow(13));//富文本消息
        chatRowHashMap.put(Integer.valueOf(14), new RichTxChatBox(14));
        chatRowHashMap.put(Integer.valueOf(15), new CardRxChatBox(15));//卡片消息
        chatRowHashMap.put(Integer.valueOf(16), new VideoRxChatRow(16));

        //物流信息相关
        chatRowHashMap.put(Integer.valueOf(17), new LogisticsInfoRxChatRow(17));//接收到的需要查询的订单列表/接收的物流进度
        chatRowHashMap.put(Integer.valueOf(18), new LogisticsInfoTxChatRow(18));//发送的商品订单

        //新卡片类型相关
        chatRowHashMap.put(Integer.valueOf(20), new NewCardInfoTxChatRow(20));//要发给坐席的卡片类型
        chatRowHashMap.put(Integer.valueOf(21), new SendCardInfoTxChatRow(21));//发出的卡片类型
        chatRowHashMap.put(Integer.valueOf(22), new ReceivedCardInfoRxChatRow(22));//收到的卡片类型
    }
