...
public class HeartSocket implements ISocket {
    ...
    public static final String ip = AppConfig.HEART_URL;

    public static final int port = 28901;//C2C
        ...
        try {
            ...
            new Thread() {
                ...
                public void run() {
                    synchronized (lock) {
                        if (socket == null) try {
                            socket = new Socket(getIP(ip), port);
    ...
    public static byte[] buildRequest(CMD cmd, byte[] body) {
        ...
        try {
            ...
            Log.i("请求heartSocket", "cmd:" + cmd.getCode() + ",length:" + length + ",sequenceId:" +
                    sequenceId );
    ...
    public void dealResponse(final TCPCallback tcpCallback) throws IOException {
        WonderfulLogUtils.logi("heartSocket","dealResponse   length");
        ...
        Log.i("返回heartSocket", "cmd:" + code + ",length:" + length + ",sequenceId:" +
...
//        }
    ...
    public String getIP(String domain) {
        ...
        try {
            ...
            e.printStackTrace();
            Log.e("socket", "获取IP失败" + e.getMessage());
        }
        return IPAddress;
