...
public class SharedPreferenceInstance {
    ...
    private static final String SP_KEY_PHONE_PASSWORLD = "SP_KEY_PHONE_PASS";
     ...
     * @param style
     */
    public void saveStyleCode(int style) {
        if (mPreferences == null) return;
        mPreferences.edit().putInt(SP_KEY_STYLE, style).apply();
    }

    public int getStyleCode() {
        return mPreferences.getInt(SP_KEY_STYLE, 1);
    }
    ...
    public void savePhoneAndPass(String phone, String pass) {
        if (mPreferences != null) {
            StringBuilder stringBuilder = new StringBuilder(phone);
            stringBuilder.append("-");
            stringBuilder.append(pass);
            mPreferences.edit().putString(SP_KEY_PHONE_PASSWORLD, stringBuilder.toString()).apply();
        }
    }

    public String[] getPhoneAndPass() {
        String[] strings = new String[2];
        if (mPreferences != null) {
            String content = mPreferences.getString(SP_KEY_PHONE_PASSWORLD, "");
