...
public class HomesAdapter extends BaseQuickAdapter<Currency, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, Currency item) {
        if (isLoad) {
            helper.setText(R.id.item_home_money, "≈" + WonderfulMathUtils.getRundNumber((item.getBaseUsdRate() == null ? 0 : item.getBaseUsdRate())*item.getClose() * MainActivity.rate,4,null)+" CNY");
        } else {
            helper.setText(R.id.item_home_money, "≈" + WonderfulMathUtils.getRundNumber((item.getBaseUsdRate() == null ? 0 : item.getBaseUsdRate())*item.getClose() * MainActivity.rate,4,null)+" CNY");
