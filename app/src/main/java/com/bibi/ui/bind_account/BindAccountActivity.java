...
public class BindAccountActivity extends BaseActivity implements BindAccountContact.View {
    ...
    protected void initViews(Bundle savedInstanceState) {
        ...
        llInternational.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                internationalClick();
            }
    ...
    private void weichatClick() {
        if (accountSetting == null) {
            WonderfulToastUtils.showToast(getResources().getString(R.string.set_account_pass));
            return;
        }
    ...
    private void internationalClick() {
        if (accountSetting == null) {
            WonderfulToastUtils.showToast(getResources().getString(R.string.set_account_pass));
            return;
        }
        BindInternationalActivity.actionStart(this,accountSetting);
    }
    ...
    public void getAccountSettingSuccess(AccountSetting obj) {
        ...
        if (accountSetting.getAliVerified() == 0) {
            tvAli.setText(getResources().getString(R.string.unbound));

        } else {
            tvAli.setText(getResources().getString(R.string.bound));

        }

        if (accountSetting.getWechatVerified() == 0) {
            tvWeiChat.setText(getResources().getString(R.string.unbound));

        } else {
            tvWeiChat.setText(getResources().getString(R.string.bound));

        }

        if (accountSetting.getBankVerified() == 0) {
            tvUnionPay.setText(getResources().getString(R.string.unbound));

        } else {
            tvUnionPay.setText(getResources().getString(R.string.bound));

        }

        //TODO 添加国际付款
        if (accountSetting.getSwiftVerified() == 0) {
            tvInternational.setText(getResources().getString(R.string.unbound));

        } else {
            tvInternational.setText(getResources().getString(R.string.bound));
