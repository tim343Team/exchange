...
import com.bibi.R;
import com.bibi.adapter.BankNameAdapter;
...
public class BindBankActivity extends BaseActivity implements BindAccountContact.BankView {
    ...
    protected void initViews(Bundle savedInstanceState) {
        ...
        if (setting.getBankVerified() == 1) {
            ...
            tvTitle.setText(getResources().getString(R.string.text_change) + getResources().getString(R.string.unionpay_account));
        } else
            tvTitle.setText(getResources().getString(R.string.binding) + getResources().getString(R.string.unionpay_account));
        ...
        if (!WonderfulStringUtils.isEmpty(name, bank, branch, account, reaccount, pwd)) {
            if (account.equals(reaccount)) {
                if (setting.getBankVerified() == 1) {
                    ...
                    presenter.getBindBank(getToken(), bank, branch, pwd, name, account);
                }
            } else
                WonderfulToastUtils.showToast(getResources().getString(R.string.diff_cardnumber));
        } else
...
//        dialog.show();
        ...
        }).setSubmitColor(Color.parseColor("#3399ff"))//确定按钮文字颜色
...
//    }
    ...
    public void getUpdateBankSuccess(String obj) {
        WonderfulToastUtils.showToast(getResources().getString(R.string.Upload_success));
        finish();
    }
