...
import com.bibi.ui.mychart.CoupleChartGestureListener;
import com.bibi.ui.mychart.DataParse;
import com.bibi.ui.mychart.KLineBean;
import com.bibi.ui.mychart.KMAEntity;
import com.bibi.ui.mychart.MyBottomMarkerView;
import com.bibi.ui.mychart.MyCombinedChart;
import com.bibi.ui.mychart.MyHMarkerView;
import com.bibi.ui.mychart.MyLeftMarkerView;
import com.bibi.ui.mychart.MyUtils;
import com.bibi.ui.mychart.VMAEntity;
import com.bibi.ui.mychart.VolFormatter;
...
import static com.bibi.ui.kline.KDataFragment.Type.MIN5;
...
//    }
    ...
    public void tcpNotify(Currency mCurrency){
        try {
            ...
            mDataTwo.setText(String.valueOf(mCurrency.getChg()));
            mDataOne.setText(String.valueOf("$" + mCurrency.getClose() + "  ↑"));
            mDataText.setText("≈" + WonderfulMathUtils.getRundNumber(mCurrency.getClose() * MainActivity.rate * (mCurrency.getBaseUsdRate() == null ? 0 : mCurrency.getBaseUsdRate()),
                    2, null) + "CNY");
        }catch (Exception e){
            e.printStackTrace();
        }
...
//        axisLeftVolume.setSpaceBottom(0);//距离顶部留白
    ...
    private synchronized void updateText(int index) {
        try {
            ...
            String openText=getResources().getString(R.string.text_it_kai);
            String highText=getResources().getString(R.string.text_it_gao);
            String lowText=getResources().getString(R.string.text_it_di);
            String closeText=getResources().getString(R.string.text_it_shou);
            String rateText=getResources().getString(R.string.text_it_zhangfu);
            tvKInfo.setText(openText+" " + MyUtils.getDecimalFormatVol(klData.open) + " "+highText+" " + MyUtils.getDecimalFormatVol(klData.high) + " "+lowText+" " +
                    MyUtils.getDecimalFormatVol(klData.low) + " "+closeText+" " + MyUtils.getDecimalFormatVol(klData.close) + " "+rateText+" " + changePercentage);
            int newIndex = index;
            if (newIndex < 4) {
...
//        combinedChart.getAxisLeft().setAxisMaxValue(kData.getVolmax());
        ...
        BarDataSet set = new BarDataSet(kData.getBarEntries(), getResources().getString(R.string.text_volume));
