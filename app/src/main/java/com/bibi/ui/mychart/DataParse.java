...
import com.bibi.utils.DateUtils;
...
//            String[] t = object.optString(i).split(" ");/*  "0930 9.50 4707",*/
    ...
    public void parseKLine(JSONArray array) {
        ...
        for (int i = 0, len = array.length(); i < len; i++) {
            ...
            String date = WonderfulDateUtils.getFormatTime("HH:mm:ss", new Date(data.optLong(0)));
            //K线实体类
            KLineBean kLineData = new KLineBean(data.optLong(0),date, (float) data.optDouble(1), (float) data.optDouble(4),
     ...
     * 将返回的k线数据转化成 自定义实体类
     */
    public KLineBean parseKLine(PushLineDataBean data) {
        String pattern = "MM-dd HH:mm:ss";
        String date = DateUtils.getFormatTime(pattern, new Date(data.getTime()));
        //K线实体类
        return new KLineBean(data.getTime(),date, (float) data.getOpenPrice(), (float) data.getClosePrice(),
                (float) data.getHighestPrice(), (float) data.getLowestPrice(), (float) data.getVolume());
