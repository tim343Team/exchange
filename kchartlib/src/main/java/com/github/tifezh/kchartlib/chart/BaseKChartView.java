...
import com.github.tifezh.kchartlib.utils.WonderfulLogUtils;
...
public abstract class BaseKChartView extends ScrollAndScaleView {
    ...
    private int mChildChildTopPadding;

    private float mMainScaleY = 1;

    private float mChildScaleY = 1;

    private float mChildChildScaleY = 1;

    private float mDataLen = 0;

    private float mCloseValue = 0;
    ...
    private float mChildChildMaxValue = Float.MAX_VALUE;

    private float mChildChildMinValue = Float.MIN_VALUE;
    ...
    private Paint mTextClosePaint = new Paint(Paint.ANTI_ALIAS_FLAG);

    private Paint mTextCloseDahPaint = new Paint();
    ...
    private IChartDraw mChildChildDraw;
    private List<IChartDraw> mChildDraws = new ArrayList<>();
    private List<IChartDraw> mChildChidDraws = new ArrayList<>();
    ...
    private void init() {
        ...
        mChildTopPadding = (int) getResources().getDimension(R.dimen.chart_top_padding);
        mChildChildTopPadding = (int) getResources().getDimension(R.dimen.chart_top_padding);
...
//        WonderfulLogUtils.logi("oldw==" + oldw + ",,oldh==" + oldh + ",,,w==" + w + ",,,h==" + h);
        this.mWidth = w;
        this.mHeight = h;
        initRect(w, h);
...
//        WonderfulLogUtils.logi("updateSize,,,w==" + w + ",,,h==" + h);
        initRect(w, h);
//        mKChartTabView.setTranslationY(mMainRect.bottom);
        setTranslateXFromScrollX(mScrollX);
        notifyChanged();
...
//            mTabRect = new Rect(0, mMainRect.bottom, mWidth, mMainRect.bottom + mMainChildSpace);
            ...
            mChildChildRect = new Rect(0, mChildRect.bottom + mChildChildTopPadding, mWidth, mChildRect.bottom + mChildChildHeight);
        } else { // 没有副图
...
//            mTabRect = new Rect(0, mMainRect.bottom, mWidth, mMainRect.bottom + mMainChildSpace);
    ...
    public float getChildChildY(float value) {
        return (mChildChildMaxValue - value) * mChildChildScaleY + mChildChildRect.top;
    }
    ...
    private void drawGird(Canvas canvas) {
        ...
        if (type != -1) {
            canvas.drawLine(0, mChildChildRect.top, mWidth, mChildChildRect.top, mGridPaint);
            canvas.drawLine(0, mChildChildRect.bottom, mWidth, mChildChildRect.bottom, mGridPaint);
        }
        ...
        for (int i = 0; i <= mGridColumns; i++) {
            ...
            if (type != -1) {
                canvas.drawLine(columnSpace * i, mChildChildRect.top, columnSpace * i, mChildChildRect.bottom, mGridPaint);
            }
        }
    }
    ...
    private void drawK(Canvas canvas) {
        ...
        for (int i = mStartIndex; i <= mStopIndex; i++) {
            ...
            if (mChildChildDraw != null && type != -1) {
                mChildChildDraw.drawTranslated(lastPoint, currentPoint, lastX, currentPointX, canvas, this, i);
            }
        }
        //画选择线
        if (isLongPress) {
            ...
            canvas.drawLine(x, mChildRect.top - mChildTopPadding, x, mChildRect.bottom, mSelectedLinePaint);
            if (mChildChildRect != null && type != -1)
                canvas.drawLine(x, mChildChildRect.top - mChildChildTopPadding, x, mChildChildRect.bottom, mSelectedLinePaint);
        }
        //还原 平移缩放
        canvas.restore();
    }
    ...
    private void drawText(Canvas canvas) {
        ...
        //--------------画上方k线图的值-------------
        if (mMainDraw != null) {
            if (mCloseValue > mMainMaxValue) {
                canvas.drawText(formatValue(mCloseValue), mWidth - mTextPaint.measureText(formatValue(mCloseValue)), baseLine + mMainRect.top, mTextPaint);
            } else {
                canvas.drawText(formatValue(mMainMaxValue), mWidth - mTextPaint.measureText(formatValue(mMainMaxValue)), baseLine + mMainRect.top, mTextPaint);
            }
            if (mCloseValue < mMainMinValue) {
                canvas.drawText(formatValue(mCloseValue), mWidth - mTextPaint.measureText(formatValue(mCloseValue)), baseLine + mMainRect.top, mTextPaint);
            } else {
                canvas.drawText(formatValue(mMainMinValue), mWidth - mTextPaint.measureText(formatValue(mMainMinValue)), mMainRect.bottom - textHeight + baseLine, mTextPaint);
            }
            ...
            for (int i = 1; i < mGridRows; i++) {
                ...
                canvas.drawText(text, mWidth - mTextPaint.measureText(text), fixTextY(rowSpace * i + mMainRect.top), mTextPaint);
            }

            //当前价格
            Log.i("mCloseValue:", mCloseValue + "");
        ...
        if (type == -1) {
            ...
            y = mChildChildRect.bottom + baseLine;
        }
    ...
    private void drawValue(Canvas canvas, int position) {
        ...
        if (position >= 0 && position < mItemCount) {
            ...
            if (mChildChildDraw != null && type != -1) {
                float y = mChildChildRect.top + baseLine - textHeight;
                float x = 0;
                mChildChildDraw.drawText(canvas, this, position, x, y);
            }
        }
    }
    ...
    private void calculateValue() {
        ...
        mChildChildMaxValue = Float.MIN_VALUE;
        mChildChildMinValue = Float.MAX_VALUE;
        ...
        for (int i = mStartIndex; i <= mStopIndex; i++) {
            ...
            if (mMainDraw != null) {
                ...
                mCloseValue = mMainDraw.getCloseValue(point);
            }
            if (mChildDraw != null) {
            ...
            if (mChildChildDraw != null && type != -1) {
                mChildChildMaxValue = Math.max(mChildChildMaxValue, mChildChildDraw.getMaxValue(point));
                mChildChildMinValue = Math.min(mChildChildMinValue, mChildChildDraw.getMinValue(point));
            }
        }
        ...
        if (mChildChildRect != null && type != -1) {
            if (mChildChildMaxValue == mChildChildMinValue) {
                //当最大值和最小值都相等的时候 分别增大最大值和 减小最小值
                mChildChildMaxValue += Math.abs(mChildChildMaxValue * 0.05f);
                mChildChildMinValue -= Math.abs(mChildChildMinValue * 0.05f);
                if (mChildChildMaxValue == 0) {
                    mChildChildMaxValue = 1;
                }
            }
            mChildChildScaleY = mChildChildRect.height() * 1f / (mChildChildMaxValue - mChildChildMinValue);
        }
     ...
     * @param stopValue  结束点的值
     */
    public void drawChildChildLine(Canvas canvas, Paint paint, float startX, float startValue, float stopX, float stopValue) {
        canvas.drawLine(startX, getChildChildY(startValue), stopX, getChildChildY(stopValue), paint);
    }
...
//        return position * 60;
     ...
     * 设置子图的绘制方法
     */
    public void setChildChildDraw(int type) {
        this.type = type;
        if (type != -1)
            this.mChildChildDraw = mChildChidDraws.get(type);
    }
...
//        mKChartTabView.addTab(name);
     ...
     * @param childDraw IChartDraw
     */
    public void addChildChidDraw(String name, IChartDraw childDraw) {
        mChildChidDraws.add(childDraw);
    }
...
//    }
     ...
     * 设置文字颜色
     */
    public void setTextClosePriceColor(int color) {
        mTextClosePaint.setColor(color);
        mTextCloseDahPaint.setColor(color);
    }
    ...
    public void setTextSize(float textSize) {
        ...
        mTextClosePaint.setTextSize(textSize);
        mTextCloseDahPaint.setTextSize(textSize);
    }
